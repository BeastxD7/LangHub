...
const TranslationForm = () => {
  ...
  const handleTranslateFile = async () => {
    ...
    try {
      ...
      setNotification('File translated successfully. You can now download the translated file.');
    } catch (error) {
      ...
      >
        ...
        <div className="flex justify-center mb-6">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={toggleMode}
            className={`px-4 py-2 rounded-md mr-2 ${!isFileMode ? 'bg-purple-600 text-white' : 'bg-gray-800 bg-opacity-50 text-gray-300'}`}
          >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={toggleMode}
            className={`px-4 py-2 rounded-md ${isFileMode ? 'bg-purple-600 text-white' : 'bg-gray-800 bg-opacity-50 text-gray-300'}`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-2">Source Language</label>
            <select
            ...
            >
              ...
              {supportedLanguages.map((lang) => (
                <option className='bg-gray-800' key={lang.code} value={lang.code}>
                  {lang.name}
                </option>
              ))}
            </select>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-2">Target Language</label>
            <select
        ...
        <AnimatePresence mode="wait">
          {!isFileMode ? (
            ...
            >
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-200 mb-2">Text to Translate</label>
                <textarea
                  value={text}
              ...
              {translatedText && (
                ...
                >
                  <h3 className="text-lg font-semibold mb-2 text-white">Translated Text</h3>
            ...
            >
              ...
              {downloadLink && (
                <motion.div 
                  ...
                  className="mt-6 bg-gray-800 bg-opacity-50 p-4 rounded-md"
                >
                  <h3 className="text-lg font-semibold mb-2 text-white">Download Translated File</h3>
                  <a 
                    href={downloadLink} 
                    target="_blank" 
