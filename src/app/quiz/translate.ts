...
export const translatePhrases = async (phrases: string[], language: string) => {
  try {
    ...
    const correctAnswers = responses.map(
    ...
    const incorrectOptionsResponses = await Promise.all(
      incorrectOptionsPromises
    );
    ...
    const questions = responses.map((response, index) => {
      ...
      const allOptions = incorrectOptionsResponses.flatMap((res) =>
        res.data[0].translations.map((translation: any) => translation.text)
      );

      // Filter out duplicates and select random options
      const uniqueOptions = Array.from(
        new Set([correctAnswer, ...allOptions])
      ).slice(0, 4);
      const options =
